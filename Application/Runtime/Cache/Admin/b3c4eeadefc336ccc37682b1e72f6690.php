<?php if (!defined('THINK_PATH')) exit();?>	<div class="content product-edit">
		<form action="<?php echo U('admin/product/save');?>" method="post" enctype="multipart/form-data">
			<h1>编辑</h1>
			<input type="hidden" name="id" value="<?php echo ($product["id"]); ?>">
			<div class="ipt-group">
				<label>产品标题:</label>
				<input name="title" type="text" placeholder="请输入标题" value="<?php echo ($product["title"]); ?>">
			</div>
			<div class="ipt-group">
				<label>产品简介:</label>
				<textarea name="brief" type="text" placeholder="简介"><?php echo ($product["brief"]); ?></textarea>
			</div>
			<div class="ipt-group">
				<label>上架下架:</label>
				<?php if($product["active"] == 1): ?><input type="radio" name="online" value="1" id="online1" style="width: 16px;height: 16px;" checked>上架
			...
			<?php if($product.banner): ?><input type="hidden" name="banner" value="<?php echo ($product["banner"]); ?>"><?php endif; ?>
			<div class="upload-img">
				<img id="imgshow" src="<?php echo ($product["banner"]); ?>">
			</div>
			<h3>正文内容</h3>
    ...
    <script type="text/javascript" charset="utf-8">  
	    ...
	    $(document).ready(function () {  
		    ...
		    //异步回调
		    ue.ready(function() {
		        UE.getEditor('prod').execCommand('insertHtml', '<?php echo ($product["content"]); ?>');
		        ue.setHeight(680);
		    });
